...
python -m absolute_zero_reasoner.main_azr_ppo \
    ...
    `# A10G OPTIMIZATION: Increased batch size for better GPU utilization` \
    data.train_batch_size=96 \
    data.val_batch_size=1312 \
    `# A10G OPTIMIZATION: Optimized sequence lengths for 24GB memory` \
    data.max_prompt_length=5120 \
    data.max_validation_prompt_length=5120 \
    data.max_response_length=6144 \
    actor_rollout_ref.model.path=Qwen/Qwen2.5-Coder-3B \
    actor_rollout_ref.actor.optim.lr=1e-6 \
    `# A10G OPTIMIZATION: Enable padding removal for better memory efficiency` \
    actor_rollout_ref.model.use_remove_padding=True \
    `# A10G OPTIMIZATION: Optimized batch sizes for 4x A10G setup` \
    actor_rollout_ref.actor.ppo_mini_batch_size=96 \
    actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=8 \
    actor_rollout_ref.actor.use_kl_loss=False \
    actor_rollout_ref.actor.kl_loss_coef=0.0 \
    actor_rollout_ref.actor.kl_loss_type=low_var_kl \
    `# A10G OPTIMIZATION: Disable sequence parallelism, use tensor parallelism` \
    actor_rollout_ref.actor.ulysses_sequence_parallel_size=1 \
    `# A10G OPTIMIZATION: Enable gradient checkpointing for memory efficiency` \
    actor_rollout_ref.model.enable_gradient_checkpointing=True \
    actor_rollout_ref.model.pretrained_tokenizer=True \
    `# A10G OPTIMIZATION: Balanced offloading strategy` \
    actor_rollout_ref.actor.fsdp_config.param_offload=False \
    actor_rollout_ref.actor.fsdp_config.grad_offload=True \
    actor_rollout_ref.actor.fsdp_config.optimizer_offload=True \
    `# A10G OPTIMIZATION: Optimized micro batch sizes` \
    actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu=32 \
    actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=32 \
    `# A10G OPTIMIZATION: 4-way tensor parallelism for 4x A10G` \
    actor_rollout_ref.rollout.tensor_model_parallel_size=4 \
    actor_rollout_ref.rollout.name=vllm \
    `# A10G OPTIMIZATION: Increased batched tokens for better throughput` \
    actor_rollout_ref.rollout.max_num_batched_tokens=20480 \
    `# A10G OPTIMIZATION: Optimized GPU memory utilization` \
    actor_rollout_ref.rollout.gpu_memory_utilization=0.45 \
    actor_rollout_ref.rollout.enforce_eager=False \
    `# A10G OPTIMIZATION: Enable cache engine for better performance` \
    actor_rollout_ref.rollout.free_cache_engine=False \
    actor_rollout_ref.rollout.n=1 \
    actor_rollout_ref.rollout.temperature=1.0 \
    `# A10G OPTIMIZATION: Enable ref model offloading` \
    actor_rollout_ref.ref.fsdp_config.param_offload=True \
    algorithm.kl_ctrl.kl_coef=0.0 \
    trainer.critic_warmup=0 \
    trainer.logger=['console','wandb'] \
    trainer.project_name='azr' \
    trainer.experiment_name='azr_coder3b_4xa10g_optimized' \
    `# A10G OPTIMIZATION: 4 GPU configuration` \
    ...
    azr.executor=qwq \
    azr.ast_check=True \
    `# A10G OPTIMIZATION: Increased reward samples for better training` \
    ...
    `# A10G OPTIMIZATION: Increased max programs for better data utilization` \
    azr.data_selection_strategy.max_programs=20480 \
